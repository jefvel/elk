...
class Uuid {
	...
	public static function v1(node: Bytes = null, optClockSequence: Int = -1, msecs: Float = -1, optNsecs: Int = -1, ?randomFunc: Void -> Int,
		...
		if (node == null) {
			node = Bytes.alloc(6);
			for (i in 0...6) node.set(i, randomFunc());
		...
		for (i in 0...6) buffer.set(i + 10, node.get(i));
	...
	public static function v3(name: String, namespace: String = "", separator: String = "-", shortUuid: Bool = false,
	...
	public static function v5(name: String, namespace: String = "", separator: String = "-", shortUuid: Bool = false,
			toAlphabet: String = FLICKR_BASE58): String {
		namespace = StringTools.replace(namespace, '-', '');
