...
class PixelPointLightShader extends hxsl.Shader {

	static var SRC = {

		@const var enableSpecular : Bool;
		
		@const var segments: Int = 5;
		...
		**/
		...
		function fragment() {
			var l = calcLighting(pixelTransformedPosition);
			var lum = min(l.r, min(l.g, l.b));
			lum *= segments;
			lum = floor(lum);
			lum /= segments;
			lightPixelColor.rgb += l * lum;
		}
	};
